...
output$data_latest <- renderText({
  ...
  exo_times <- exo %>%
    ...
    pull(TIMESTAMP)
  formatted_date_exo <- ifelse(length(exo_times) > 0,
                               paste(
                                 format(
                                   max(exo_times, na.rm = T),
                                   "%Y-%m-%d %H:%M:%S"
                                 ),
                                 TIMEZONE
                               ),
                               "No data")
  exo_color <- if (formatted_date_exo == "No data" ||
                   now() - max(exo_times, na.rm = T) > hours(1)) {
    ' style="color: #990000;"'
  } else {
    ""
  }
  ...
  radon_times <- radon %>%
    filter(!if_any(-TIMESTAMP, is.na)) %>%
    pull(TIMESTAMP)
  formatted_date_rad <- ifelse(length(radon_times) > 0,
                               paste(
                                 format(
                                   max(radon_times, na.rm = T),
                                   "%Y-%m-%d %H:%M:%S"
                                 ),
                                 TIMEZONE
                               ),
                               "No data")
  radon_color <- if (formatted_date_rad == "No data" ||
                     now() - max(radon_times, na.rm = T) > hours(1)) {
    ' style="color: #990000;"'
  } else {
    ""
  }
  ...
  hydrology_times <- hydrology %>%
    filter(!if_any(-TIMESTAMP, is.na)) %>%
    pull(TIMESTAMP)
  formatted_date_hydrology <- ifelse(length(hydrology_times) > 0,
                                     paste(
                                       format(
                                         max(hydrology_times, na.rm = T),
                                         "%Y-%m-%d %H:%M:%S"
                                       ),
                                       TIMEZONE
                                     ),
                                     "No data")
  hydrology_color <- if (formatted_date_hydrology == "No data" ||
                         now() - max(hydrology_times, na.rm = T) > hours(1)) {
    ' style="color: #990000;"'
  } else {
    ""
  }
  ...
  formatted_list <- paste(
    ...
    <li", hydrology_color, "><b>SontekIQ ADCP:</b> ", formatted_date_hydrology, "</li>
