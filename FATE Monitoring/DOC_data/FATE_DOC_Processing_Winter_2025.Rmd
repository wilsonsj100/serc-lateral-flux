---
title: "FATE Monitoring: DOC Data - May want to rerun these samples"
author: "Winter 2025"
date: "`r Sys.Date()`"
output: 
  pdf_document:
...
###### Run information - PLEASE CHANGE
  Date_Run = "03/19/26"  #Date that instrument was run
  Run_by = "Stephanie J. Wilson"  #Instrument user 
  Script_run_by = "Stephanie J. Wilson" #Code user 
  run_notes = "Check standards and duplicates were off on this run."  #any notes from the run
...
#file path and name for raw summary data file 
    raw_file_name = "Raw Data/FATE_DOC_202603.txt" 
    
#file path and name for raw all peaks file 
    raw_allpeaks_name = "Raw Data/FATE_DOC_202603_allpeaks.txt"

#file path and name of processed data file 
    processed_file_name = "Processed Data/FATE_Processed_DOC_202601.csv"
...
stds_all$area <- as.numeric(stds_all$area)
...
stds_N <- stds_all %>%
  ...
  mutate(run_date = as.Date(strptime(run_datetime, format = "%m/%d/%Y %I:%M:%S %p"))) %>%
  #### ONly for this run because the standard curve is getting separated because one ran after midnight 
  mutate(
    run_date = if_else(
      standard_N_ppm == 3 & run_date == as.Date("2026-03-18"),
      as.Date("2026-03-17"),
      run_date
    )
  )
...
C_stds_plot <- ggplot(stds_C, aes(x = standard_C_ppm, y = area)) +
  ...
  geom_text(
    data = r2_labels_c,
    aes(x = x_pos, y = y_pos, label = paste("R2 = ", r_squared)),
    inherit.aes = FALSE,
    hjust = 1, vjust = 1,
    size = 4
  ) +
...
N_stds_plot <- ggplot(stds_N, aes(x = standard_N_ppm, y = area)) +
  ...
  facet_wrap(~ run_date) +
    geom_text(
    data = r2_labels_n,
    aes(x = x_pos, y = y_pos, label = paste0("R2 = ", r_squared)),
    inherit.aes = FALSE,
    hjust = 1, vjust = 1,
    size = 4
  ) +
