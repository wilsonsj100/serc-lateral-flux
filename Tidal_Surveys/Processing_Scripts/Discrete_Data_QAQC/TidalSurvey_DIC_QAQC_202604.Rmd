---
title: "Tidal Survey: 2026-04 DIC QAQC"
author: "2025 Winter Samples"
date: "`r Sys.Date()`"
output: 
  pdf_document:
    toc: true
...
#file path and name for raw summary data file 
    raw_file_name = here("Tidal_Surveys/Raw_Data/202604" , "TidalSurvey_DIC_202604.txt" )
    
#file path and name for raw all peaks file 
    raw_allpeaks_name = here("Tidal_Surveys/Raw_Data/202604" , "TidalSurvey_DIC_202604_allpeaks.txt")

#file path and name of processed data file 
    processed_file_name = here("Tidal_Surveys/Processed_Data/202604" , "TidalSurvey_DIC_Processed_202604_allpeaks.csv") 
...
#qaqc log file path for this year 
  Log_path = here("FATE Monitoring/DIC_data/Raw Data" , "FATE_DIC_QAQClog_2025.csv") 
...
C_stds_plot <- ggplot(stds_C, aes(x = standard_C_ppm, y = area)) +
  ...
  labs(
    title = "Std Curve by Date",
    x = "Inorganic Carbon Standard Concentration (ppm)",
    y = "Peak Area"
  ) +
  theme_bw()
...
## Summarize and add in metadata 
```{r}

cat("Average across analytical duplicates")
...
#need to read in metadata 
#head(dic_metadata)
#dic_metadata <- dic_metadata %>%
  #mutate(sample_name_1 = gsub("_(A|B)", "", as.character(DIC_A_new))) #%>% 
    #mutate(sample_name_1 = recode(sample_name_1,
    #"MD_PW_2_syringe" = "MD_PW_14_syringe"  ))

#dic_metadata_selected <- dic_metadata %>%
 # select(DIC_A_new, DIC_B_new, sample_name_1, installation_id, sample_id, timezone, 
    #     sample_collection_date,
     #    sample_collection_time, filter_date, filter_time) 

#merge metadata with sample run data 
#merged_data <- dat_summary %>%
 # left_join(dic_metadata_selected, by = c("sample_name_1" = "sample_name_1"))
